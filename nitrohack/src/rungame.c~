...
nh_bool loadgame(void)
{
    ...
    if (!get_gamedir(SAVE_DIR, savedir)) {
	curses_raw_print("找不到或创建不了游戏保存目录。");
	return FALSE;
    }
    
    files = list_gamefiles(savedir, &size);
    if (!size) {
	curses_msgwin("没有游戏保存记录。");
	return FALSE;
    }
    ...
    n = curses_display_menu(items, icount, "保存的游戏", PICK_ONE, pick);    
    free(items);
    filename[0] = '\0';
    if (n > 0)
	...
	destroy_game_windows();
	close(fd);
	if (curses_yn_function("载入游戏失败，是否删除这个记录？", "yn", 'n') == 'y')
	    unlink(filename);
	return FALSE;
    }
